...
method_dropdown = [{"label":"Single Stage Multiclass Model", "value": "Method1"}, 
                   {"label":"Two Stage Model", "value": "Method2"}, 
                   {"label":"Single Stage Binary Models", "value": "Method3"}]
            ...
            [
                ...
                html.Div([
                 ...
                 html.Div([
                     ...
                     html.Div(id = "classification_metrics", 
                               style={
                            "width": "100%",
                            "height": "400px",
                            "display": "inline-block",
...
[Output('feature-graphic', 'figure'),
 ...
 Output('classification_metrics', 'children')],
...
def update_confusion_matrix(n_clicks,to_keep_classes, frequency_target, std_var_gene_seq_len, number_unique_low_level, skewness_coef, 
    ...
    if ml_method == "BRF": 
        ...
        rf = BalancedRandomForestClassifier(n_jobs = 6, random_state = 84)
        rf1 = BalancedRandomForestClassifier(n_jobs = 6, random_state = 84)
    else: 
        rf = RandomForestClassifier(n_jobs = 6, random_state = 84)
        rf1 = RandomForestClassifier(n_jobs = 6, random_state = 84)
    ...
    if method == "Method1":
        ...
        combo = pd.concat([part1, other_part]).reindex(data.index)
        ...
        return px.imshow(df_cm, text_auto=True, color_continuous_scale='RdBu_r', 
                         ...
                         title = "Confusion Matrix for Single Stage Multiclass Model (average accuracy)"), \
               px.imshow(df_cm_std, text_auto=True, color_continuous_scale='RdBu_r', 
                         labels=dict(y="True Label", x="Predicted Label"), 
                         title = "Confusion Matrix for Single Stage Multiclass Model (standard errors)"), \
               ...
               px.imshow(df_cm_std, text_auto=True, color_continuous_scale='RdBu_r', 
                         labels=dict(y="True Label", x="Predicted Label"), 
                         title = "Confusion Matrix for Single Stage Multiclass Model (standard errors)"), \
               dbc.Table.from_dataframe(overall_df)
...
#             test_data_inner["y_pred_" + classes] = y_pred
        ...
        return px.imshow(df_cm, text_auto=True, color_continuous_scale='RdBu_r', 
                         labels=dict(y="True Label", x="Predicted Label"), 
                         title = "Confusion Matrix for Single Stage Binary Model (average accuracy)"), \
               px.imshow(df_cm_std, text_auto=True, color_continuous_scale='RdBu_r', 
                         labels=dict(y="True Label", x="Predicted Label"), 
                         title = "Confusion Matrix for Single Stage Binary Model (standard errors)"), \
               px.imshow(df_cm, text_auto=True, color_continuous_scale='RdBu_r', 
                         labels=dict(y="True Label", x="Predicted Label"), 
                         title = "Confusion Matrix for Single Stage Binary Model (average accuracy)"), \
               px.imshow(df_cm_std, text_auto=True, color_continuous_scale='RdBu_r', 
